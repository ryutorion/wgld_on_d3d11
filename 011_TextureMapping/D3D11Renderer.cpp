...
b8 D3D11Renderer::LoadTexture()
{
	...
	hr = mpDevice->CreateTexture2D1(&texture2d_desc, &subresource_data, &mpTexture);
	if(FAILED(hr))
	{
		return false;
	}
	...
	hr = mpDevice->CreateSamplerState(&sampler_desc, &mpSamplerState);
	if(FAILED(hr))
	{
		return false;
	}
...
b8 D3D11Renderer::SetupGraphicsPipeline()
{
	...
	ID3D11ShaderResourceView * pp_SRVs[]{ mpShaderResourceView.Get()};
